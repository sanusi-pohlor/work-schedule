...
<template>
  <div class="flex flex-col min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-blue-200">
    <Header />
    <main class="flex-grow container mx-auto p-4 max-w-7xl">
      <!-- Search & Filter Bar -->
      <div class="bg-white p-4 rounded-3xl mb-8 border border-gray-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="relative w-full md:w-1/2">
          ...
          <input 
            ...
            type="text" 
            class="block w-full p-3 pl-11 text-sm text-slate-700 border border-gray-300 rounded-2xl bg-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 transition-all duration-300 shadow-inner" 
            placeholder="Search tasks..." 
          />
        </div>
        <div class="flex items-center space-x-3 w-full md:w-auto">
          <label class="text-sm font-semibold text-slate-500 whitespace-nowrap">Filter:</label>
          <select 
            v-model="store.filterStatus"
            class="bg-white border border-gray-300 text-slate-700 text-sm rounded-2xl focus:ring-blue-500 focus:border-blue-500 block w-full md:w-auto p-2.5 transition-all duration-300 shadow-sm font-medium"
          >
      ...
      <div class="mb-10 relative">
        ...
        <draggable
          class="flex overflow-x-auto py-6 space-x-6 min-h-[160px] p-4 rounded-3xl bg-gray-100 border border-gray-200 shadow-inner custom-scrollbar"
      ...
      <div class="inline-flex items-center justify-center w-full mb-10 relative">
        ...
        <div class="absolute px-4 -translate-x-1/2 left-1/2 bg-slate-50 text-gray-400">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <!-- Received Column -->
        <div class="bg-white p-6 rounded-3xl border border-gray-200 shadow-md flex flex-col">
          <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
            ...
            <span class="bg-blue-50 text-xs px-3 py-1.5 rounded-full text-blue-700 font-bold border border-blue-100">{{ store.filteredReceivedCards.length }}</span>
          </div>
          <draggable
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-200 shadow-md flex flex-col">
          <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
            <h2 class="text-xl font-bold flex items-center gap-2 text-amber-500">
              <span class="w-3 h-3 rounded-full bg-amber-400 shadow-sm"></span>
              In Progress
            </h2>
            <span class="bg-amber-50 text-xs px-3 py-1.5 rounded-full text-amber-700 font-bold border border-amber-100">{{ store.filteredInProgressCards.length }}</span>
          </div>
          <draggable
            class="flex-grow min-h-[200px] space-y-4 rounded-xl"
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-200 shadow-md flex flex-col">
          <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
            ...
            <span class="bg-emerald-50 text-xs px-3 py-1.5 rounded-full text-emerald-700 font-bold border border-emerald-100">{{ store.filteredCompletedCards.length }}</span>
          </div>
          <draggable
            class="flex-grow min-h-[200px] space-y-4 rounded-xl"
...
<style>
/* Custom scrollbar for horizontal board - Light */
.custom-scrollbar::-webkit-scrollbar {
  height: 8px;
}
.custom-scrollbar::-webkit-scrollbar-track {
  background: rgba(229, 231, 235, 0.5); /* gray-200 */
  border-radius: 10px;
}
